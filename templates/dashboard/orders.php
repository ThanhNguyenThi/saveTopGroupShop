 ...
 *
 * Override this template by copying it to yourtheme/groupshops/dashboard/
 *
 * @package    groupshops
 * @version    1.0.0
 */
...
jQuery(function () {
    jQuery('a.view-items').on('click', function (e) {
        ...
        if ( jQuery(this).text() == "<?php _e('Hide items', 'topgroupshops'); ?>" ) {
        	jQuery(this).text("<?php _e('View items', 'topgroupshops'); ?>");
    	} else {
        	jQuery(this).text("<?php _e('Hide items', 'topgroupshops'); ?>");
    	}
...
<table class="table table-condensed table-vendor-sales-report">
	<thead>
	<tr>
	<th class="product-header"><?php _e( 'Order', 'topgroupshops' ); ?></th>
	<th class="quantity-header"><?php _e( 'Shipping', 'topgroupshops' ) ?></th>
	<th class="commission-header"><?php _e( 'Total', 'topgroupshops' ) ?></th>
	<th class="rate-header"><?php _e( 'Date', 'topgroupshops' ) ?></th>
	<th class="rate-header"><?php _e( 'Links', 'topgroupshops' ) ?></th>
	</thead>
	<tbody>
	 ...
	 ?>
		...
		<?php foreach ( $order_summary as $order ) :
			...
			<tr id="order-<?php echo $order->id; ?>" data-order-id="<?php echo $order->id; ?>">
				...
				<td><?php echo apply_filters( 'topgroupshops_dashboard_google_maps_link', '<a target="_blank" href="' . esc_url( 'http://maps.google.com/maps?&q=' . urlencode( esc_html( preg_replace( '#<br\s*/?>#i', ', ', $order->get_formatted_shipping_address() ) ) ) . '&z=16' ) . '">'. esc_html( preg_replace( '#<br\s*/?>#i', ', ', $order->get_formatted_shipping_address() ) ) .'</a>' ); ?></td>
				<td><?php $sum = TGS_Queries::sum_for_orders( array( $order->id ), array('vendor_id'=>get_current_user_id()) ); $total = $sum[0]->line_total; $totals += $total; echo woocommerce_price( $total ); ?></td>
				...
				$order_actions = array(
					'view'		=> array(
						'class' 	=> 'view-items',
						'content'	=> __('View items', 'topgroupshops'),
					)
				);
				if ( $needs_shipping ) {
					$order_actions['shipped'] = array(
						'class' 	=> 'mark-shipped',
						'content'	=> __('Mark shipped', 'topgroupshops'),
				...
				if ( $shipped ) {    
					$order_actions[ 'shipped' ] = array(
					'class' 	=> 'mark-shipped',
					'content'	=> __('Shipped', 'topgroupshops'),
					'url'		=> '#'
					);
				} 
				...
				if ( $providers && $needs_shipping && class_exists( 'WC_Shipment_Tracking' ) ) {
					$order_actions['tracking'] = array(
						...
						'content'	=> __( 'Tracking', 'topgroupshops' )
					);
				}
				
				$order_actions = apply_filters( 'topgroupshops_order_actions', $order_actions, $order );          
	...
	<?php else : ?>
		...
		<tr>
			<td colspan="4"
				style="text-align:center;"><?php _e( 'You have no orders during this period.', 'topgroupshops' ); ?></td>
		</tr>

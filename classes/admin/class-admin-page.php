...
class TGS_Admin_Setup
{
	...
	{
		add_filter( 'set-screen-option', array( 'TGS_Admin_Setup', 'set_table_option' ), 10, 3 );
		add_action( 'admin_menu', array( 'TGS_Admin_Setup', 'menu' ) );
		...
		add_filter( 'woocommerce_debug_tools', array( $this, 'topgroupshops_tools' ) ); 
	...
	public function add_vendor_details( $order )
	{
		...
		echo '<h4>' . __('Vendors shipped', 'topgroupshops') . '</h4><br/>';
	...
	public function append_actions( $order, $order_page = false )
	{
		...
		$authors = TGS_Function_Vendors::get_vendors_from_order( $order );
		...
		foreach ($authors as $author ) {
			 ...
			 $string .= TGS_Function_Vendors::get_vendor_shop_name( $author );
			 $string .= '</br>';
		}

		$response = array(
			'url'       => '#',
			'name'      => __('Vendors Shipped', 'topgroupshops') . $string,
			'action'    => 'wc_pv_shipped',
			'image_url' => tgs_assets_url . '/images/icons/truck.png',
		);
	...
	public static function menu()
	{
		$hook = add_submenu_page(
			'woocommerce',
			__( 'Commission', 'topgroupshops' ), __( 'Commission', 'topgroupshops' ),
			'manage_woocommerce',
			'pv_admin_commissions',
			array( 'TGS_Admin_Setup', 'commissions_page' )
		);

		add_action( "load-$hook", array( 'TGS_Admin_Setup', 'add_options' ) );

		add_action( "admin_print_styles-$hook", 	array( 'TGS_Admin_Setup', 'commission_enqueue_style' ) );
		add_action( "admin_print_scripts-$hook", 	array( 'TGS_Admin_Setup', 'commission_my_enqueue_script' ) );
	 ...
	 * @access public 
	 */
	public function topgroupshops_tools( $tools ){ 

		$tools[ 'reset_topgroupshops_roles' ] = array(
				'name'    => __( 'Reset TGS Vendors roles ', 'topgroupshops' ),
				'button'  => __( 'Reset TGS Vendor Roles', 'topgroupshops' ),
				'desc'    => __( 'This will reset the topgroupshops roles ( vendor & pending_vendor ), back to the default capabilities.', 'topgroupshops' ),
				'callback' => array( 'TGS_Admin_Setup', 'reset_vendor_roles' )
			); 

		$tools[ 'reset_topgroupshops' ] = array(
				'name'    => __( 'Reset TGS Vendors ', 'topgroupshops' ),
				'button'  => __( 'Reset TGS Vendors Settings', 'topgroupshops' ),
				'desc'    => __( 'This will reset topgroupshops back to defaults. This DELETES ALL YOUR Settings.', 'topgroupshops' ),
				'callback' => array( 'TGS_Admin_Setup', 'reset_topgroupshops' )
			); 
	...
	public static function reset_vendor_roles(){ 
		...
		$can_add          = TGS_Vendors::$pv_options->get_option( 'can_submit_products' );
		$can_edit         = TGS_Vendors::$pv_options->get_option( 'can_edit_published_products' );
		$can_submit_live  = TGS_Vendors::$pv_options->get_option( 'can_submit_live_products' );
		$can_view_reports = TGS_Vendors::$pv_options->get_option( 'can_view_backend_reports' );
		...
		echo '<div class="updated inline"><p>' . __( 'TGS Vendor roles successfully reset.', 'topgroupshops' ) . '</p></div>';
	 ...
	 * @access public
	 */	
	public static function reset_topgroupshops(){ 

		delete_option( TGS_Vendors::$id . '_options' ); 
		echo '<div class="updated inline"><p>' . __( 'TGS Vendors was successfully reset. All settings have been reset.', 'topgroupshops' ) . '</p></div>';
	...
	public static function commission_enqueue_style(){ 
		...
		wp_enqueue_style( 'commissions_select2_css', tgs_assets_url . 'css/select2.min.css' );

	} //commission_enqueue_style() 

	public static function commission_my_enqueue_script(){ 

		$select2_args = apply_filters( 'topgroupshops_select2_commission_args', array( 
			'placeholder' => __( 'Select a Vendor', 'topgroupshops' ), 
			'allowclear' => true, 
		) ); 

		wp_enqueue_script( 'commissions_select2_styles_js', tgs_assets_url. 'js/select2.min.js', array('jquery') );
		
		wp_register_script( 'commissions_select2_load_js', tgs_assets_url. 'js/tgs-commissions.js', array('jquery') );
		wp_localize_script( 'commissions_select2_load_js', 'tgs_commissions_select', $select2_args ); 
	...
	public static function add_options()
	{
		...
		$PV_Admin_Page = new TGS_Admin_Page();
	...
	public static function commissions_page()
	{
		...
		<div class="wrap">
			...
			<h2><?php _e( 'Commission', 'topgroupshops' ); ?></h2>
 ...
 */
class TGS_Admin_Page extends WP_List_Table
{
	...
	function column_default( $item, $column_name )
	{
		...
		switch ( $column_name ) {
			...
			case 'vendor_id' :
				...
				return '<a href="' . admin_url( 'user-edit.php?user_id=' . $item->vendor_id ) . '">' . TGS_Function_Vendors::get_vendor_shop_name( $item->vendor_id ) . '</a>';
			case 'total_due' :
			...
			case 'product_id' :
				...
				$tgs_total_sales = get_post_meta( $product_id, 'total_sales', true );
                return '<a href="' . admin_url( 'post.php?post=' . $product_id . '&action=edit' ) . '">' . get_the_title( $item->product_id ) . '</a> (<span title="' . get_the_title( $item->product_id ) .' has sold ' . $tgs_total_sales . ' times total.">' . $tgs_total_sales  . '</span>)';
			case 'order_id' :
	...
	function get_columns()
	{
		$columns = array(
			...
			'product_id' 		=> __( 'Product', 'topgroupshops' ),
			'order_id'   		=> __( 'Order ID', 'topgroupshops' ),
			'vendor_id' 	    => __( 'Vendor', 'topgroupshops' ),
			'total_due'  		=> __( 'Commission', 'topgroupshops' ),
			'total_shipping'  	=> __( 'Shipping', 'topgroupshops' ),
			'tax'  				=> __( 'Tax', 'topgroupshops' ),
			'totals'  			=> __( 'Total', 'topgroupshops' ),
			'status'     		=> __( 'Status', 'topgroupshops' ),
			'time'       		=> __( 'Date', 'topgroupshops' ),
		);
	...
	{
		$actions = array(
			'mark_paid'     => __( 'Mark paid', 'topgroupshops' ),
			'mark_due'      => __( 'Mark due', 'topgroupshops' ),
			'mark_reversed' => __( 'Mark reversed', 'topgroupshops' ),
			// 'delete' => __('Delete', 'topgroupshops'),
		);
		
		$actions = apply_filters('tgs_edit_bulk_actions', $actions);
	...
	function months_dropdown( $post_type )
	{
		...
		<select name="m" id="filter-by-date">
			<option<?php selected( $m, 0 ); ?> value='0'><?php _e( 'Show all dates', 'topgroupshops' ); ?></option>
			<?php
	...
	function status_dropdown( $post_type )
	{
		...
		<select name="com_status">
			<option<?php selected( $com_status, '' ); ?> value=''><?php _e( 'Show all Statuses', 'topgroupshops' ); ?></option>
			<option<?php selected( $com_status, 'due' ); ?> value="due"><?php _e( 'Due', 'topgroupshops' ); ?></option>
			<option<?php selected( $com_status, 'paid' ); ?> value="paid"><?php _e( 'Paid', 'topgroupshops' ); ?></option>
			<option<?php selected( $com_status, 'reversed' ); ?> value="reversed"><?php _e( 'Reversed', 'topgroupshops' ); ?></option>
		</select>
	<?php
	}
	...
	function process_bulk_action()
	{
		...
		switch ( $this->current_action() ) {
			case 'mark_paid':
				...
				if ( $result )
					echo '<div class="updated"><p>' . __( 'Commission marked paid.', 'topgroupshops' ) . '</p></div>';
				break;
			...
			case 'mark_due':
				...
				if ( $result )
					echo '<div class="updated"><p>' . __( 'Commission marked due.', 'topgroupshops' ) . '</p></div>';
				break;
			...
			case 'mark_reversed':
				...
				if ( $result )
					echo '<div class="updated"><p>' . __( 'Commission marked reversed.', 'topgroupshops' ) . '</p></div>';
				break;

			default:
				// code...
				do_action('tgs_edit_process_bulk_actions', $this->current_action(), $ids);
				break;
		}

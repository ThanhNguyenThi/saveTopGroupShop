 ...
 * Shipping functions
 *
 * @author  Thanh Nguyen <http://topgroupshops.com>
...
class TGS_Shipping
{
	...
	function __construct()
	{
		...
		if ( function_exists( 'woocommerce_get_shipping_method_table_rate' ) ) {
			...
			add_action( 'woocommerce_checkout_update_order_meta', array( 'TGS_Shipping', 'trs2_add_shipping_data' ), 1, 2 );
			add_action( 'wc_trs2_matched_rates', array( 'TGS_Shipping', 'trs2_store_shipping_data' ), 10, 3 );
		}
	}
	...
	public static function get_shipping_due( $order_id, $product, $author )
	{
		...
		if ( $_product && $_product->needs_shipping() && !$_product->is_downloadable() ) {
			...
			if ( ( class_exists('WC_Shipping_Per_Product_Init') || function_exists( 'woocommerce_per_product_shipping' ) ) && $method == 'per_product' ) {
				$shipping_costs = TGS_Shipping::pps_get_due( $order_id, $product );
				...
				if ( $local_delivery[ 'type' ] == 'product' ) {
					...
					$shipping_costs['tax'] 		= TGS_Shipping::calculate_shipping_tax( $shipping_costs['amount'], $order ); 
				}
				...
				if ( $int_delivery[ 'type' ] == 'item' ) {
					...
					$shipping_costs['tax'] 				= ( 'taxable' === $int_delivery[ 'tax_status' ] ) ? TGS_Shipping::calculate_shipping_tax( $shipping_costs['amount'], $order ) : 0; 
				}

			}
		}

		$shipping_costs = apply_filters( 'topgroupshops_shipping_due', $shipping_costs, $order_id, $product, $author );
	...
	public static function pps_get_due( $order_id, $product )
	{
		...
		$shipping_costs['tax'] = ('taxable' === $taxable ) ? TGS_Shipping::calculate_shipping_tax( $item_shipping_cost, $order ) : 0; 
	...
	public function trs2_get_due( $order_id, $product_id )
	{
		...
		TGS_Shipping::trs2_retrieve_shipping_data( $order_id );
		if ( !empty( TGS_Shipping::$trs2_shipping_calc_type ) ) {

			$ship_id = ( TGS_Shipping::$trs2_shipping_calc_type == 'class' ) ? get_product( $product_id )->get_shipping_class_id() : $product_id;

			if ( !empty( TGS_Shipping::$trs2_shipping_rates[ $ship_id ] ) ) {
				$shipping_due = TGS_Shipping::$trs2_shipping_rates[ $ship_id ];
				unset( TGS_Shipping::$trs2_shipping_rates[ $ship_id ] );
			}
		}
	...
	public function trs2_retrieve_shipping_data( $order_id )
	{
		...
		if ( !empty( TGS_Shipping::$trs2_shipping_rates ) ) return;

		TGS_Shipping::$trs2_shipping_rates     = array_filter( (array) get_post_meta( $order_id, '_topgroupshops_trs2_shipping_rates', true ) );
		TGS_Shipping::$trs2_shipping_calc_type = get_post_meta( $order_id, '_topgroupshops_trs2_shipping_calc_type', true );
	}
	...
	public function trs2_add_shipping_data( $order_id, $post )
	{
		...
		foreach ( $woocommerce->session->trs2_shipping_rates as $key => $shipping_rates ) {
			...
			if ( is_array( $shipping_rates ) && array_sum( $shipping_rates ) == $order->order_shipping ) {
				...
				update_post_meta( $order_id, '_topgroupshops_trs2_shipping_rates', $shipping_rates );
				update_post_meta( $order_id, '_topgroupshops_trs2_shipping_calc_type', $shipping_calc_type );
